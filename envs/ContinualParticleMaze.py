...
class ContinualParticleMaze():
    ...
    def __init__(self, grid_name, dense=False):
        ...
        self.metadata = None  # compatibility with stable baselines
        ...
        for i in range(self.grid_size):
            for j in range(self.grid_size):
                if self.grid[i, j] == True:
                    ...
                    cur_ind = self.get_index(self.x)
                    if (cur_ind[0],cur_ind[1])== (i, j):
                        print('A', end='')
                    elif (self.goal_ind[0], self.goal_ind[1]) == (i, j):
                        print('G', end='')
                    elif self.start_ind is not None and (self.start_ind[0],
                                                     self.start_ind[1]) == (i, j):
                        print('S', end='')
                    else:
                        print(' ', end=(''))
            print('\n', end='')
